...
void BMSModuleManager::decodevwtemp(BMS_CAN_MESSAGE &msg, int debug)
{
  int CMU = (msg.id & 0xFF);
  if (debug == 1)
  {
...
void BMSModuleManager::decodevwcan(BMS_CAN_MESSAGE &msg, int debug)
{
  int CMU, Id = 0;
  switch (msg.id)
  {
  ...
  if (CMU > 0 && CMU < 64)
  {
    ...
    modules[CMU].setExists(true);
    modules[CMU].setReset(true);
    modules[CMU].decodevwcan(Id, msg);
  }
}


void BMSModuleManager::decodebmwcan(BMS_CAN_MESSAGE &msg, int debug)
{
  int Id = (msg.id & 0x0F0);
